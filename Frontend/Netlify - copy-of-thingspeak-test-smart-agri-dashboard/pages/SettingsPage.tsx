...
const SettingsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 bg-card p-4 sm:p-6 rounded-xl shadow-lg">
      <h2 className="text-xl sm:text-2xl font-semibold text-textDark">{t('settings.pageTitle')}</h2>
      
      <div className="border-b border-gray-200 pb-4">
        <h3 className="text-base sm:text-lg font-medium text-gray-900">{t('settings.profile.title')}</h3>
        ...
        <div className="mt-4 space-y-3">
            <div>
                ...
                <input type="text" name="fullName" id="fullName" defaultValue={t('header.adminUser')} className="mt-1 block w-full sm:w-3/4 md:w-1/2 border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary text-sm p-2"/>
            </div>
            <div>
                <label htmlFor="email" className="block text-sm font-medium text-gray-700">{t('settings.profile.label.emailAddress')}</label>
                <input type="email" name="email" id="email" defaultValue="admin@smartagri.example.com" className="mt-1 block w-full sm:w-3/4 md:w-1/2 border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary text-sm p-2"/>
            </div>
            <button type="button" className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-dark w-full sm:w-auto">
      ...
      <div className="border-b border-gray-200 pb-4 pt-4">
        <h3 className="text-base sm:text-lg font-medium text-gray-900">{t('settings.notifications.title')}</h3>
