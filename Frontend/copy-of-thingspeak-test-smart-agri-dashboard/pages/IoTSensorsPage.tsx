    ...
    <div className="space-y-6">
      ...
      {sensors.length > 0 && (
        <>
          ...
          {sensorsForHistoricalCharts.length > 0 && (
            <div className="mt-8">
              <h3 className="text-xl font-semibold text-textDark mb-4">{t('iotSensors.historicalData.titleAll')}</h3>
              <div className="flex overflow-x-auto space-x-6 pb-4 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
                ...
                {sensorsForHistoricalCharts.map(sensor => {
                  if (!sensor.historicalData || sensor.historicalData.length === 0) return (
                    <div key={`${sensor.id}-chart-wrapper-nodata`} ref={el => { chartCardRefs.current[sensor.type] = el; }} className="min-w-[400px] flex-shrink-0">
                  ...
                  return (
                    <div key={`${sensor.id}-chart-wrapper`} ref={el => { chartCardRefs.current[sensor.type] = el; }} className="min-w-[400px] flex-shrink-0">
                        <ChartCard title={t(config.titleKey)} subtitle={t(config.subtitleKey, config.subtitleValues)}>
                          <ResponsiveContainer width="100%" height="100%">
                            <ChartComponent data={sensor.historicalData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                              ...
                              <XAxis dataKey="timestamp" tickFormatter={formatChartTimestamp} tick={{ fill: '#6B7280', fontSize: 12 }} />
                              <YAxis tick={{ fill: '#6B7280', fontSize: 12 }} unit={config.yAxisUnit} domain={config.yAxisDomain as [number | string, number | string] | undefined} allowDataOverflow={true} />
                              <Tooltip labelFormatter={formatChartTimestamp} contentStyle={{ backgroundColor: 'white', border: '1px solid #ccc', borderRadius: '4px' }} />
                              <Legend wrapperStyle={{ fontSize: '12px' }} />
                              {config.lines.map(line => (
                                config.chartType === 'LineChart' ? (
                                  <Line key={line.dataKey} type="monotone" dataKey={line.dataKey} name={t(line.nameKey)} stroke={line.strokeColor} strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} connectNulls={false} />
                                ) : (
                                  <Area key={line.dataKey} type="monotone" dataKey={line.dataKey} name={t(line.nameKey)} stroke={line.strokeColor} fillOpacity={1} fill={`url(#${line.fillId})`} strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} connectNulls={false} />
                                )
                              ))}
                            </ChartComponent>
                          </ResponsiveContainer>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('iotSensors.table.header.sensorType')}</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('iotSensors.table.header.currentValue')}</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('iotSensors.table.header.unit')}</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('iotSensors.table.header.status')}</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {sensors.map(sensor => {
                    ...
                    const statusColorClass = getStatusTextColorClass(statusKey);

                    return (
                      <tr key={sensor.id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{getTranslatedSensorType(sensor.type)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{displayValue}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{displayUnit}</td>
